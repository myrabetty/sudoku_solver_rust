...
// returns the cell with a value that is to be added in the grid.
    pub(crate) fn find_new_guess(allowed_values: &Vec<EmptyCell>) -> Result<Guess, ()> {
        ...
        debug!("allowed values before applying two pints strategy {:?}", allowed_values);
...
// only one allowed value.
    ...
    fn apply_two_cells_strategy(initial_allowed_values: &Vec<EmptyCell>) -> Option<Guess> {
        ...
        let mut allowed_values = initial_allowed_values.clone();
        for value in 1..10_u8 {
            for quadrant in 0..9 {
