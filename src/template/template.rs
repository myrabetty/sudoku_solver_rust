    ...
    for i in 0..9 {
        for j in 0..9 {
            ...
            if grid[[i, j]].value != 0 {
                ...
                let index = allowed_values.iter().position(|x| x.row == i && x.column == j).expect(&*format!("failed to find value of row {:} and column {:}", i, j));
...
mod tests {
    ...
    fn display_current_grid_state_test() {
        ...
        allowed_values.push(EmptyCell::new(0, 1, vec![1, 5,9]));
        ...
        allowed_values.push(EmptyCell::new(1, 1, vec![1, 5]));
        ...
        allowed_values.push(EmptyCell::new(2, 5, vec![2,5, 6, 9]));
        ...
        allowed_values.push(EmptyCell::new(5, 5, vec![1, 7]));
        ...
        allowed_values.push(EmptyCell::new(7, 5, vec![1, 7]));
